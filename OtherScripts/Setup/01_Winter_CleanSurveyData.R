...
Winter$How.would.you.describe.your.gender. <-
  as.numeric(ifelse(
    Winter$How.would.you.describe.your.gender. == unique(Winter$How.would.you.describe.your.gender.)[1],
    1,
    ifelse(
      Winter$How.would.you.describe.your.gender. == unique(Winter$How.would.you.describe.your.gender.)[2],
      0,
      2
    )
  ))
colnames(Winter)[which(names(Winter) == "How.would.you.describe.your.gender.")] <-
  "Gender" ## RENAME COLUMNS
Winter <-
  Winter[Winter$Gender != 2, ] ## Drop "In Another Way" respondents for simplicity
...
Winter$ExactAge <-
  as.numeric(ifelse(
    is.na(
      Winter$In.what.year.were.you.born....Please.select.from.the.dropdown.box
    ) == TRUE,
    ifelse(
      Winter$How.old.are.you. == unique(Winter$How.old.are.you.)[1],
      23,
      ifelse(
        Winter$How.old.are.you. == unique(Winter$How.old.are.you.)[2],
        65,
        ifelse(
          Winter$How.old.are.you. == unique(Winter$How.old.are.you.)[3],
          45,
          ifelse(
            Winter$How.old.are.you. == unique(Winter$How.old.are.you.)[4],
            35,
            ifelse(
              Winter$How.old.are.you. == unique(Winter$How.old.are.you.)[5],
              55,
              70
            )
          )
    ...
    as.numeric(
      2021 - Winter$In.what.year.were.you.born....Please.select.from.the.dropdown.box
    )
  ))
Winter$DummyAge <-
  as.numeric(ifelse(Winter$ExactAge >= median(Winter$ExactAge, na.rm = TRUE), 1, 0))
...
Winter$MilesDistance <-
  recode(
    Winter$Please..indicate.the.approximate.distance.as.the.crow.flies..one.way..between.your.home..and.your.nearby.woodland.,
    "Less than 1 mile" = 1,
    "1-4 miles" = 2.5,
    "5-9 miles" = 7,
    "10-19 miles" = 14.5,
...
Winter$PostcodeDistance <-
  ud.convert(Winter$PostcodeDistance, "mi", "km")
...
colnames(LevelsB2A) <-
  paste(colnames(LevelsB2A), "B") # The columns wouldn't merge unless the new headers were different so I change the BLOCKTWO column names


## Take all the respondent choices and change into a single vector
Choices <- data.frame(Choice = c(t(data.frame(
  rep(Winter[, 33:41] , times = 1)
))[, ]))


Winter_NoCE <-
  Winter[, -c(33:41)] ## Necessary to reshape survey data.
...
Reshaped <- (data.frame(rep((1:9), times = nrow(Winter)),
                        Choices,
                        data.frame(apply(X = Winter_NoCE, 2, function(x)
                          rep(x, each = 9)))))
colnames(Reshaped)[1:2] <-
  c("Task", "Choice") ## Note: The `Task` variable refers to which number of choice tasks the respondent did.
...
# ***********************
Reshaped$Sound1 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Sound1, LevelsB2A$`Sound1 B`)
Reshaped$Sound2 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Sound2, LevelsB2A$`Sound2 B`)
Reshaped$Sound3 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Sound3, LevelsB2A$`Sound3 B`)
Reshaped$Colour1 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Colour1, LevelsB2A$`Colour1 B`)
Reshaped$Colour2 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Colour2, LevelsB2A$`Colour2 B`)
Reshaped$Colour3 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Colour3, LevelsB2A$`Colour3 B`)

Reshaped$Smell1 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Smell1, LevelsB2A$`Smell1 B`)
Reshaped$Smell2 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Smell2, LevelsB2A$`Smell2 B`)
Reshaped$Smell3 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Smell3, LevelsB2A$`Smell3 B`)

Reshaped$Deadwood1 <-
  ifelse(Reshaped$Block == 1,
         LevelsB1A$Deadwood1,
         LevelsB2A$`Deadwood1 B`)
Reshaped$Deadwood2 <-
  ifelse(Reshaped$Block == 1,
         LevelsB1A$Deadwood2,
         LevelsB2A$`Deadwood2 B`)
Reshaped$Deadwood3 <-
  ifelse(Reshaped$Block == 1,
         LevelsB1A$Deadwood3,
         LevelsB2A$`Deadwood3 B`)

Reshaped$Tax1 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Tax1, LevelsB2A$`Tax1 B`)
Reshaped$Tax2 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Tax2, LevelsB2A$`Tax2 B`)
Reshaped$Tax3 <-
  ifelse(Reshaped$Block == 1, LevelsB1A$Tax3, LevelsB2A$`Tax3 B`)
...
database <-
  database[!is.na(database$MilesDistance), ] ## Drop missing distances
...
database$WoodlandsScore <-
  rowMeans(
    cbind(
      database$WoodlandsSounds,
      database$WoodlandsSmells,
      database$WoodlandsColours,
      database$WoodlandsTree
    )
  )
Winter$WoodlandsScore <-
  rowMeans(
    cbind(
      Winter$WoodlandsSounds,
      Winter$WoodlandsSmells,
      Winter$WoodlandsColours,
      Winter$WoodlandsTree
    )
  )
colnames(Winter)[which(names(Winter) == "Spring20210WoodlandVisitDummy")] <-
  "MostRecentVisit"
colnames(database)[which(names(database) == "Spring20210WoodlandVisitDummy")] <-
  "MostRecentVisit"
