...
# Function: The fun interactions model one. Table B7
# Notes: should work
...
apollo_control = list(
  ...
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelOne_Correlated_InteractionsInUtility",
  modelName  = "Winter_MXL_ModelOne_Correlated_InteractionsInUtility", ## Added dates last verified
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['A']]  = asc_A  + beta_Tax*(Tax1 +
    ...
    Interaction_Smell_Impairment * (SmellIssues * (Smell1 == 1)) +
    Interaction_Sound_Impairment * (HearingIssues * (Sound1 == 1)) +
    Interaction_Colour_Impairment * (SightIssues * (Colour1 == 1)) +
    Interaction_Smell2_Impairment * (SmellIssues * (Smell1 == 2)) +
    Interaction_Sound2_Impairment * (HearingIssues * (Sound1 == 2)) +
    Interaction_Colour2_Impairment * (SightIssues * (Colour1 == 2))
  ...
  V[['B']]  = asc_B  + beta_Tax*(Tax2 +
    ...
    Interaction_Smell_Impairment * (SmellIssues * (Smell2 == 1)) +
    Interaction_Sound_Impairment * (HearingIssues * (Sound2 == 1)) +
    Interaction_Colour_Impairment * (SightIssues * (Colour2 == 1)) +
    Interaction_Smell2_Impairment * (SmellIssues * (Smell2 == 2)) +
    Interaction_Sound2_Impairment * (HearingIssues * (Sound2 == 2)) +
    Interaction_Colour2_Impairment * (SightIssues * (Colour2 == 2))
...
Winter_MXL_ModelOne_Correlated_InteractionsInUtility = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelOne_Correlated_InteractionsInUtility,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_MXL_ModelOne_Correlated_InteractionsInUtility,saveOutput_settings = list(printPVal=TRUE))
saveRDS(Winter_MXL_ModelOne_Correlated_InteractionsInUtility, here("CEoutput/ModelOne","Winter_MXL_ModelOne_Correlated_InteractionsInUtility.rds"))
...
Model <- readRDS(here("CEoutput/ModelOne","Winter_MXL_ModelOne_Correlated_InteractionsInUtility.rds")) ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelOne_Correlated_InteractionsInUtility_ConWTP <- apollo_conditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_Correlated_InteractionsInUtility_ConWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_Correlated_InteractionsInUtility_ConWTP.csv"))


## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
Winter_MXL_ModelOne_Correlated_InteractionsInUtility_UnconWTP <- apollo_unconditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_Correlated_InteractionsInUtility_UnconWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_Correlated_InteractionsInUtility_UnconWTP.csv"))


# *********************************************************************************************************
#### END OF SCRIPT ####
## Next Step: 13_Winter_Table1_SampleTests.R
# *********************************************************************************************************
