...
apollo_control = list(
  nCores    = 10,
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelOne_PrefSpace_Interactions",
  modelName  = "Winter_MXL_ModelOne_PrefSpace_Interactions", ## Added dates last verified
...
apollo_beta = c(
  ...
  Interaction_Smell_Impairment = 0,
  Interaction_Sound_Impairment = 0,
  Interaction_Colour_Impairment = 0,
  Interaction_Deadwood_Impairment = 0,
  Interaction_Smell2_Impairment = 0,
  Interaction_Sound2_Impairment = 0,
  Interaction_Colour2_Impairment = 0,
  Interaction_Deadwood2_Impairment = 0
)
...
apollo_randCoeff = function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["b_Smell"]] =  (mu_Smell + sig_Smell * draws_Smell + Interaction_Smell_Impairment * SmellIssues)
  randcoeff[["b_Sound"]] =  (mu_Sound + sig_Sound * draws_Sound + Interaction_Sound_Impairment * HearingIssues)
  randcoeff[["b_Colour"]] =  (mu_Colour + sig_Colour * draws_Colour   + Interaction_Colour_Impairment * SightIssues)
  randcoeff[["b_Deadwood"]] =  (mu_Deadwood + sig_Deadwood * draws_Deadwood   + Interaction_Deadwood_Impairment * Impairment)
  randcoeff[["b_Smell2"]] =  (mu_Smell2 + sig_Smell2 * draws_Smell2 + Interaction_Smell2_Impairment * SmellIssues)
  randcoeff[["b_Sound2"]] =  (mu_Sound2 + sig_Sound2 * draws_Sound2  + Interaction_Sound2_Impairment * HearingIssues)
  randcoeff[["b_Colour2"]] =  (mu_Colour2 + sig_Colour2 * draws_Colour2  + Interaction_Colour2_Impairment * SightIssues)
  randcoeff[["b_Deadwood2"]] =  (mu_Deadwood2 + sig_Deadwood2 * draws_Deadwood2   + Interaction_Deadwood2_Impairment * Impairment)
  return(randcoeff)
}
...
Winter_MXL_ModelOne_PrefSpace_Interactions = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelOne_PrefSpace_Interactions,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_MXL_ModelOne_PrefSpace_Interactions,saveOutput_settings = list(printPVal=TRUE))
saveRDS(Winter_MXL_ModelOne_PrefSpace_Interactions, here("CEoutput/ModelOne","Winter_MXL_ModelOne_PrefSpace_Interactions.rds"))
...
Model <- readRDS(here("CEoutput/ModelOne","Winter_MXL_ModelOne_PrefSpace_Interactions.rds")) ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelOne_PrefSpace_Interactions_ConWTP <- apollo_conditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_PrefSpace_Interactions_ConWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_PrefSpace_Interactions_ConWTP.csv"))


## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
Winter_MXL_ModelOne_PrefSpace_Interactions_UnconWTP <- apollo_unconditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_PrefSpace_Interactions_UnconWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_PrefSpace_Interactions_UnconWTP.csv"))
