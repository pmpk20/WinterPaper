...
apollo_control = list(
  nCores    = 10,
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelTwo_ASCNormal",
  modelName  = "Winter_MXL_ModelTwo_ASCNormal", ## Added dates last verified
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['A']]  = asc_A  + beta_Tax*(Tax1 +
                                   b_Sound  * (Sound1 == 1) + b_Sound2  * (Sound1 ==
                                                                             2) +
                                   b_Smell * (Smell1 == 1) + b_Smell2 *
                                   (Smell1 == 2) +
                                   b_Colour * (Colour1 == 1) + b_Colour2 * (Colour1 ==
                                                                              2) +
                                   b_Deadwood * (Deadwood1 == 7) + b_Deadwood2 *
                                   (Deadwood1 == 15) )
...
Winter_MXL_ModelTwo_ASCNormal = apollo_estimate(apollo_beta,
                                      apollo_fixed,
...
apollo_modelOutput(Winter_MXL_ModelTwo_ASCNormal,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_MXL_ModelTwo_ASCNormal,saveOutput_settings = list(printPVal=TRUE))
saveRDS(Winter_MXL_ModelTwo_ASCNormal, here("CEoutput/ModelTwo","Winter_MXL_ModelTwo_ASCNormal.rds"))
...
#
# Model <- readRDS(here("CEoutput/ModelTwo","Winter_MXL_ModelTwo_ASCNormal.rds")) ## Enter model of interest RDS here
#
#
# ## Calculate conditional WTP:
# Winter_MXL_ModelTwo_ASCNormal_ConWTP <-
#   apollo_conditionals(Model, apollo_probabilities, apollo_inputs)
# fwrite(
#   Winter_MXL_ModelTwo_ASCNormal_ConWTP %>% data.frame(),
#   sep = ",",
#   here("CEoutput/ModelTwo", "Winter_MXL_ModelTwo_ASCNormal_ConWTP.csv")
# )
#
#
# ## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
# Winter_MXL_ModelTwo_ASCNormal_UnconWTP <-
#   apollo_unconditionals(Model, apollo_probabilities, apollo_inputs)
# fwrite(
#   Winter_MXL_ModelTwo_ASCNormal_UnconWTP %>% data.frame(),
#   sep = ",",
#   here("CEoutput/ModelTwo", "Winter_MXL_ModelTwo_ASCNormal_UnconWTP.csv")
# )
