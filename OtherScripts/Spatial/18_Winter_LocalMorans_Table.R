...
LocalMoransTable <- rbind.fill(
  data.frame(rbind(round(
    table(Data$quad_sig_WTP_Tax), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_Tax)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_Colour), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_Colour)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_ColourHigh), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_ColourHigh)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_Smell), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_Smell)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_SmellHigh), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_SmellHigh)
    ) * 100, 2), "%)")
  ))),


  data.frame(rbind(round(
    table(Data$quad_sig_WTP_Sound), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_Sound)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_SoundHigh), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_SoundHigh)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_Decomposition), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_Decomposition)
    ) * 100, 2), "%)")
  ))),

  data.frame(rbind(round(
    table(Data$quad_sig_WTP_DecompositionHigh), 2
  ),
  noquote(
    paste0("(", round(prop.table(
      table(Data$quad_sig_WTP_DecompositionHigh)
    ) * 100, 2), "%)")
  )))
)
...
# ******************************************************************************
#### Section 4: Create output tables ####
# ******************************************************************************
...
# Rearrange a little
Table4_LocalMorans <- cbind("Variable" = rep(RowLabels, each = 2),
                            Table4_LocalMorans)


colnames(Table4_LocalMorans) <-
  c("Variable", "Low:Low", "Not Signif.", "X5", "High:High")
...
Table4_LocalMorans %>% write.csv(quote = FALSE, row.names = FALSE)

## Output to a discrete file if that's helpful
write.table(
  Table4_LocalMorans,
  here("OtherOutput/Spatial", "Table4_LocalMorans.txt"),
  sep = ",",
  quote = FALSE
)
