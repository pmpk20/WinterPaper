#### WP5 Winter Paper: Model Two - Test ####
## Function: Estimate Mixed Logit on Attributes With Covariates
## Author: Dr Peter King (p.king1@leeds.ac.uk)
## Last change: 18/10/2023
## Changes:
## - Woodlands score as quantile
...
database$WoodlandsScoreDummy <-
  ifelse(database$WoodlandsScore > median(database$WoodlandsScore),
         0,
         1)

database$WoodlandsScoreQuantile <- cut(
  database$WoodlandsScore,
  breaks = seq.int(from = 0, to = 100, by = 20) %>% c(),
  labels = c(0, 1, 2, 3, 4)
) %>% as.numeric()
...
apollo_control = list(
  nCores    = 10,
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelTwo_WoodlandTest_Quantile",
  modelName  = "Winter_MXL_ModelTwo_WoodlandTest_Quantile", ## Added dates last verified
  indivID    = "Respondent", ## This is the name of a column in the database indicating each unique respondent
  outputDirectory = "CEoutput/ModelTwo/Robustness"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  asc_C1 = asc_C + beta_Gender * Gender +
    beta_Age * DummyAge +
    ...
    beta_WoodlandsScore * WoodlandsScoreQuantile
...
Winter_MXL_ModelTwo_WoodlandTest_Quantile = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelTwo_WoodlandTest_Quantile,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_MXL_ModelTwo_WoodlandTest_Quantile,saveOutput_settings = list(printPVal=TRUE))
saveRDS(Winter_MXL_ModelTwo_WoodlandTest_Quantile, here("CEoutput/ModelTwo","Winter_MXL_ModelTwo_WoodlandTest_Quantile.rds"))
...
Model <- readRDS(here("CEoutput/ModelTwo","Winter_MXL_ModelTwo_WoodlandTest_Quantile.rds")) ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelTwo_WoodlandTest_Quantile_ConWTP <-
  apollo_conditionals(Model, apollo_probabilities, apollo_inputs)
fwrite(
  Winter_MXL_ModelTwo_WoodlandTest_Quantile_ConWTP %>% data.frame(),
  sep = ",",
  here("CEoutput/ModelTwo", "Winter_MXL_ModelTwo_WoodlandTest_Quantile_ConWTP.csv")
)


## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
Winter_MXL_ModelTwo_WoodlandTest_Quantile_UnconWTP <-
  apollo_unconditionals(Model, apollo_probabilities, apollo_inputs)
fwrite(
  Winter_MXL_ModelTwo_WoodlandTest_Quantile_UnconWTP %>% data.frame(),
  sep = ",",
  here("CEoutput/ModelTwo", "Winter_MXL_ModelTwo_WoodlandTest_Quantile_UnconWTP.csv")
)
...
# Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP <- data.frame(fread(here("CEoutput/ModelTwo","Winter_MXL_ModelTwo_WoodlandTest_Quantile_ConWTP.csv")))
# Winter_MXL_ModelTwo_WoodlandTest_QuantileSummary <-data.frame(cbind("TaxWTP_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$beta_Tax.post.mean,
#                                                              "SmellWTP_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Smell.post.mean,
#                                                              "SmellWTP2_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Smell2.post.mean,
#                                                              "SoundWTP_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Sound.post.mean,
#                                                              "SoundWTP2_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Sound2.post.mean,
#                                                              "ColourWTP_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Colour.post.mean,
#                                                              "ColourWTP2_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Colour2.post.mean,
#                                                              "DeadwoodWTP_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Deadwood.post.mean,
#                                                              "DeadwoodWTP2_Test"=Winter_MXL_ModelTwo_WoodlandTest_Quantile_WTP$b_Deadwood2.post.mean))


# Summary <- Winter_MXL_ModelTwo_WoodlandTest_QuantileSummary %>% summarise(across(everything(),list(mean)))
...
# AttributeWTPDensity <- Winter_MXL_ModelTwo_WoodlandTest_QuantileSummary %>%
#   melt() %>%
...
#        filename = paste0("Winter_Figure2_ModelTwo_DensityPlot_",Date,".jpeg"),
...
# Winter <- cbind(Winter,
#                 Winter_MXL_ModelTwo_WoodlandTest_QuantileSummary)
#
