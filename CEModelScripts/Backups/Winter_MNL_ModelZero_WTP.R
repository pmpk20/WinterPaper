...
apollo_control = list(
  modelDescr = "Winter_MNL_ModelZero_WTP",
  modelName  = "Winter_MNL_ModelZero_WTP", ## Added dates last verified
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  # V[["A"]] = beta_Tax * (Tax1 + b_Sound * (Sound1 == 1) + b_Sound2 *
  #                          (Sound1 == 2) + b_Smell * (Smell1 == 1) + b_Smell2 *
  #                          (Smell1 == 2) + b_Colour * (Colour1 == 1) + b_Colour2 *
  #                          (Colour1 == 2) + b_Deadwood * (Deadwood1 == 7) + b_Deadwood2 *
  #                          (Deadwood1 == 15))
  # V[["B"]] = +beta_Tax * (Tax2 + b_Sound * (Sound2 == 1) +
  #                           b_Sound2 * (Sound2 == 2) + b_Smell * (Smell2 == 1) +
  #                           b_Smell2 * (Smell2 == 2) + b_Colour * (Colour2 == 1) +
  #                           b_Colour2 * (Colour2 == 2) + b_Deadwood * (Deadwood2 = 7) + b_Deadwood2 * (Deadwood2 == 15))
...
Winter_MNL_ModelZero_WTP = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MNL_ModelZero_WTP,modelOutput_settings = list(printPVal=TRUE))



apollo_saveOutput(Winter_MNL_ModelZero_WTP,saveOutput_settings = list(printPVal=TRUE))
# saveRDS(Winter_MNL_ModelZero_WTP, here("CEoutput/ModelOne","Winter_MNL_ModelZero_WTP.rds"))
...
nsim <- 1000
betas <- Winter_MNL_ModelZero_WTP$estimate[3:12]
vcmat <- Winter_MNL_ModelZero_WTP$robvarcov
