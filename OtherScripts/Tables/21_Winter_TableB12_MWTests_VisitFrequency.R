...
# Function: Mann-Whitney tests between all frequencies of forest visits
# Notes: should work
...
# - Adding table
# - Changing to report mean(SD) WTP
...
WTP <-
  WTP[WTP %>% dplyr::select(-ends_with(c(".ID", ".post.sd"))) %>% colnames()] %>% data.frame()
...
## Recategorise here
Data$MostRecentVisit_Never <-
  ifelse(Data$MostRecentVisit == 0, 1, 0)
Data$MostRecentVisit_Season <-
  ifelse(Data$MostRecentVisit == 1, 1, 0)
Data$MostRecentVisit_Month <-
  ifelse(Data$MostRecentVisit == 2, 1, 0)
Data$MostRecentVisit_Week <- ifelse(Data$MostRecentVisit == 3, 1, 0)
Data$MostRecentVisit_NearWeek <-
  ifelse(Data$MostRecentVisit == 4, 1, 0)
Data$MostRecentVisit_Daily <-
  ifelse(Data$MostRecentVisit == 5, 1, 0)


## Define categories to loop through
Right <- c(
  "MostRecentVisit_Never",
  "MostRecentVisit_Season",
  "MostRecentVisit_Month",
  "MostRecentVisit_Week",
  "MostRecentVisit_NearWeek",
  "MostRecentVisit_Daily"
)
...
for (i in seq_along(Left)) {
  for (j in seq_along(Right)) {
    Test <- wilcox.test(formula = as.formula(paste0(Left[i],
                                                    " ~ ",
                                                    Right[j])),
                        data = Data)
    # Test <- wilcox.test(formula = as.formula(paste0(Left[i],
    ...
    # ES <- rstatix::wilcox_effsize(formula = as.formula(paste0(Left[i],
    #                                                           " ~ ",
    #                                                           Right[j])),
    #                               data = Data)
    ES <-
    ...
    results_inner[[j]] <-     rbind(
      Test$data.name,
      ES$n1 %>% round(3),
      ES$n2 %>% round(3),
      # Data %>%
      #   filter(Data[[Right[j]]] == 1) %>%
      #   summarise_at(
      #     vars(Left[i]),
      #     list(mean = mean, sd = sd),
      #     na.rm = TRUE
      #   ) %>%
      #   mutate(mean_sd = glue("{sprintf(fmt = '%.3f', mean)} ({sprintf(fmt = '%.3f', sd)})")) %>%
      #   pull(mean_sd),
      Test$statistic %>% round(3),
      Test$p.value %>% AddStars(),
      ES$effsize %>% round(3) %>% sprintf("%.3f", .)
    )
  }
  results_outer[[i]] <- results_inner
}


Output <- do.call(bind_cols, results_outer) %>% t() %>% data.frame()
...
TableB12 %>% fwrite(
  ...
  here("OtherOutput/Tables",
       "TableB21_MWTests_VisitFrequency.txt"),
  row.names = TRUE,
  quote = FALSE
)
...
# **********************************************************************************
#### END OF SCRIPT ####
## Next step: 22_Winter_FigureB1_Impairments.R
# **********************************************************************************
