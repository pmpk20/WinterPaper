...
GB_Winter_Joined <-
  left_join(x = GB_Winter, PopDensity, by = c("NUTS" = "NUTS"))
GB_Winter_Joined <-
  left_join(x = GB_Winter_Joined, GDHI, by = c("NUTS" = "NUTS"))


## Little column renaming to make things easier
colnames(GB_Winter_Joined)[which(names(GB_Winter_Joined) == colnames(GB_Winter_Joined)[291])] <-
  "Density"


## Easiest way to add data on the trees:
GB_Winter_Joined <-
  cbind(GB_Winter_Joined, as.data.frame(Trees)[, 1:4])
