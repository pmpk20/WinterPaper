...
# Function: To estimate the WTP-space MXL in Table B5, specification Model One
# Notes: should work
...
Winter_MXL_ModelOne = apollo_estimate(apollo_beta,
                                      apollo_fixed,
                                      apollo_probabilities,
                                      apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelOne, modelOutput_settings = list(printPVal =
                                                                      TRUE))
apollo_saveOutput(Winter_MXL_ModelOne, saveOutput_settings = list(printPVal =
                                                                    TRUE))
saveRDS(Winter_MXL_ModelOne,
        here("CEoutput/ModelOne", "Winter_MXL_ModelOne.rds"))
...
Model <- here("CEoutput/ModelOne",
              "Winter_MXL_ModelOne.rds") %>%
  readRDS() ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelOne_ConWTP <-
  apollo_conditionals(Model, apollo_probabilities, apollo_inputs)

fwrite(
  Winter_MXL_ModelOne_ConWTP %>% data.frame(),
  sep = ",",
...
Winter_MXL_ModelOne_UnconWTP <-
  apollo_unconditionals(Model, apollo_probabilities, apollo_inputs)


fwrite(
  Winter_MXL_ModelOne_UnconWTP %>% data.frame(),
  sep = ",",
...
# *********************************************************************************************************
#### END OF SCRIPT ####
## Next Step: 07_Winter_MXL_ModelTwo_WTPSpace.R
# *********************************************************************************************************
