...
apollo_control = list(
  ...
  mixing    = TRUE,
  modelDescr = "Winter_Test_PrefSpaceUncorrelated",
  modelName  = "Winter_Test_PrefSpaceUncorrelated", ## Added dates last verified
...
Winter_Test_PrefSpaceUncorrelated = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_Test_PrefSpaceUncorrelated,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_Test_PrefSpaceUncorrelated,saveOutput_settings = list(printPVal=TRUE))
# saveRDS(Winter_Test_PrefSpaceUncorrelated, here("CEoutput/ModelOne","Winter_Test_PrefSpaceUncorrelated.rds"))
...
# Model <- readRDS(here("CEoutput/ModelOne","Winter_Test_PrefSpaceUncorrelated.rds")) ## Enter model of interest RDS here

#
# ## Calculate conditional WTP:
# Winter_Test_PrefSpaceUncorrelated_ConWTP <- apollo_conditionals(Model,apollo_probabilities,apollo_inputs )
# fwrite(Winter_Test_PrefSpaceUncorrelated_ConWTP %>% data.frame(),sep=",",
#        here("CEoutput/ModelOne","Winter_Test_PrefSpaceUncorrelated_ConWTP.csv"))
#
#
# ## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
# Winter_Test_PrefSpaceUncorrelated_UnconWTP <- apollo_unconditionals(Model,apollo_probabilities,apollo_inputs )
# fwrite(Winter_Test_PrefSpaceUncorrelated_UnconWTP %>% data.frame(),sep=",",
#        here("CEoutput/ModelOne","Winter_Test_PrefSpaceUncorrelated_UnconWTP.csv"))
