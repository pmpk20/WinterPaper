...
Winter <- here("OtherData", "Winter_dataframe_Step4_NONANONYMISED.csv") %>% fread() %>% data.frame()
...
SpatialLagModel <- function(Model, WTP,Data, K) {
  ...
  SLM_dummy_Attribute <- spatialreg::lagsarlm(
    formula = Model,
    data=Data,
    NearestNeighbour)
  SLM_dummy_Attribute_S <- summary(SLM_dummy_Attribute)
  OLS_dummy_Attribute <- lm(formula = Model,
                            data=Data) # Dummy regression
...
for (i in 1:length(Dependent)) {
  ...
  Model <- reformulate(termlabels = Independent,
              response = Dependent[i])
...
ModelOutput <- function(Model,Result) {
  ...
  rbind(
    ...
    data.frame("Value" = c("LM"),
    "Data" = paste(ifelse(
      Result$LMlag$p.value < 0.01,
      paste0(Result$LMlag$statistic %>% round(3) %>% sprintf("%.3f", .), "***"),
      ifelse(
        Result$LMlag$p.value < 0.05,
        paste0(Result$LMlag$statistic %>% round(3) %>% sprintf("%.3f", .), "**"),
        ifelse(
          Result$LMlag$p.value < 0.1,
          paste0(Result$LMlag$statistic %>% round(3) %>% sprintf("%.3f", .), "*"),
          Result$LMlag$statistic %>% round(3) %>% sprintf("%.3f", .)
        )
      )
    ))),
    ...
    #              paste0(Estimates$LR1$statistic %>% round(3) %>% sprintf("%.3f", .), "***"),
    #              ifelse(
    ...
    #                  paste0(Estimates$LR1$statistic %>% round(3) %>% sprintf("%.3f", .), "*"),
    #                  Estimates$LR1$statistic %>% round(3) %>% sprintf("%.3f", .)
    #                )
    #              )
    #            ))),

    data.frame("Value" = c("Rho"),
               "Data" =
...
Table7 %>% data.frame() %>%
  fwrite(sep=",",
       here::here("OtherOutput/Tables",
                  "Table7_SpatialLagModels.txt"))
