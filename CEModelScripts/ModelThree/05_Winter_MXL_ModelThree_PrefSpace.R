...
apollo_control = list(
  nCores    = 10,
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelThree_PrefSpace",
  modelName  = "Winter_MXL_ModelThree_PrefSpace",
...
Winter_MXL_ModelThree_PrefSpace = apollo_estimate(apollo_beta,
                                                  apollo_fixed,
                                                  apollo_probabilities,
                                                  apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelThree_PrefSpace,
                   modelOutput_settings = list(printPVal = TRUE))
apollo_saveOutput(Winter_MXL_ModelThree_PrefSpace,
                  saveOutput_settings = list(printPVal = TRUE))
saveRDS(
  Winter_MXL_ModelThree_PrefSpace,
  here("CEoutput/ModelTwo", "Winter_MXL_ModelThree_PrefSpace.rds")
)
...
Model <-
  readRDS(here("CEoutput/ModelTwo", "Winter_MXL_ModelThree_PrefSpace.rds")) ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelThree_PrefSpace_ConWTP <-
  apollo_conditionals(Model, apollo_probabilities, apollo_inputs)
fwrite(
  Winter_MXL_ModelThree_PrefSpace_ConWTP %>% data.frame(),
  sep = ",",
  here(
    "CEoutput/ModelTwo",
    "Winter_MXL_ModelThree_PrefSpace_ConWTP.csv"
  )
)


## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
Winter_MXL_ModelThree_PrefSpace_UnconWTP <-
  apollo_unconditionals(Model, apollo_probabilities, apollo_inputs)
fwrite(
  Winter_MXL_ModelThree_PrefSpace_UnconWTP %>% data.frame(),
  sep = ",",
  here(
    "CEoutput/ModelTwo",
    "Winter_MXL_ModelThree_PrefSpace_UnconWTP.csv"
  )
)
...
# **********************************************************************************
#### END OF SCRIPT ####
## Next step: 06_Winter_MXL_ModelTwo_WTPSpace.R
# **********************************************************************************
