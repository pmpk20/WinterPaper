...
apollo_control = list(
  ...
  mixing    = TRUE,
  modelDescr = "H3_Winter_PrefSpaceModel",
  modelName  = "H3_Winter_PrefSpaceModel",
  indivID    = "Respondent",
...
H3_Winter_PrefSpaceModel = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)


## Model output and results here alongside saving information
apollo_modelOutput(H3_Winter_PrefSpaceModel,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(H3_Winter_PrefSpaceModel,saveOutput_settings = list(printPVal=TRUE))
# saveRDS(H3_Winter_PrefSpaceModel, file="H3_Winter_PrefSpaceModel.rds")
...
#Model <- here("CEOutputData/H3", "H3_Winter_PrefSpaceModel_model.rds") %>% readRDS()


## Calculate conditional WTP:
H3_Winter_PrefSpaceModel_ConWTP <- apollo_conditionals(H3_Winter_PrefSpaceModel,apollo_probabilities,apollo_inputs )
fwrite(
  H3_Winter_PrefSpaceModel_ConWTP %>% data.frame(),
  sep = ",",
  here("CEoutput/ModelOne", "H3_Winter_PrefSpaceModel_ConWTP.csv")
)

WTP <- H3_Winter_PrefSpaceModel_ConWTP %>% data.frame()
...
ggsave(
  ...
  filename = paste0(
    here(),
    "/OtherOutput/Figures/",
    "H3_Winter_PrefSpaceModel_ConWTP_Plot.png"
  ),
  width = 20,
  height = 15,
  units = "cm",
