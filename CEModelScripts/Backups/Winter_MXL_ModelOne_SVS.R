...
apollo_control = list(
  nCores    = 10,
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelOne_SVS_V0",
  modelName  = "Winter_MXL_ModelOne_SVS_V0", ## Added dates last verified
...
apollo_draws = list(
  ...
  interNDraws    = 10000, ## Same results if you use 5000 draws
  interUnifDraws = c(),
  interNormDraws = c(
    "draws_Tax",
    "draws_Smell",
...
Winter_MXL_ModelOne_SVS_V0 = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(Winter_MXL_ModelOne_SVS_V0,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(Winter_MXL_ModelOne_SVS_V0,saveOutput_settings = list(printPVal=TRUE))
saveRDS(Winter_MXL_ModelOne_SVS_V0, here("CEoutput/ModelOne","Winter_MXL_ModelOne_SVS_V0.rds"))
...
Model <- readRDS(here("CEoutput/ModelOne","Winter_MXL_ModelOne_SVS_V0.rds")) ## Enter model of interest RDS here


## Calculate conditional WTP:
Winter_MXL_ModelOne_SVS_V0_ConWTP <- apollo_conditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_SVS_V0_ConWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_SVS_V0_ConWTP.csv"))


## Calculate unconditional WTP: (Needed for Fig.2. of the paper) [NOTE: Unconditionals make v large dataframes]
Winter_MXL_ModelOne_SVS_V0_UnconWTP <- apollo_unconditionals(Model,apollo_probabilities,apollo_inputs )
fwrite(Winter_MXL_ModelOne_SVS_V0_UnconWTP %>% data.frame(),sep=",",
       here("CEoutput/ModelOne","Winter_MXL_ModelOne_SVS_V0_UnconWTP.csv"))
