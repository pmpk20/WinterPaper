...
# Function: To estimate model2 for Table B4
# Notes: should work
...
apollo_control = list(
  ...
  mixing    = TRUE,
  modelDescr = "Winter_MXL_ModelTwo_PrefSpace",
  modelName  = "Winter_MXL_ModelTwo_PrefSpace",
  indivID    = "Respondent",
...
apollo_draws = list(
  interDrawsType = "pmc",
  ## Robust to using MLHS or Sobol draws
  interNDraws    = 1000,
  ## Same results if you use 5000 draws
  interUnifDraws = c(),
  interNormDraws = c(
    "draws_Tax",
    "draws_Smell",
...
apollo_probabilities = function(apollo_beta,
  ...
  V[['B']]  = asc_B  + beta_Tax * Tax2 +
    b_Sound  * (Sound2 == 1) + b_Sound2  * (Sound2 ==
                                              2)  +
    b_Smell * (Smell2 == 1) + b_Smell2 *
    (Smell2 == 2) +
    b_Colour * (Colour2 == 1) + b_Colour2 * (Colour2 ==
                                               2) +
    b_Deadwood * (Deadwood2 == 7) + b_Deadwood2 *
    (Deadwood2 == 15)
  V[['C']]  = asc_C1
...
Winter_MXL_ModelTwo_PrefSpace = apollo_estimate(
  apollo_beta,
  apollo_fixed,
...
apollo_modelOutput(Winter_MXL_ModelTwo_PrefSpace,
                   modelOutput_settings = list(printPVal = TRUE))
apollo_saveOutput(Winter_MXL_ModelTwo_PrefSpace,
                  saveOutput_settings = list(printPVal = TRUE))
# saveRDS(Winter_MXL_ModelTwo_PrefSpace, file="Winter_MXL_ModelTwo_PrefSpace.rds")
...
Model <-
  here("CEoutput/ModelTwo",
       "Winter_MXL_ModelTwo_PrefSpace_model.rds") %>% readRDS()
...
apollo_unconditionals(Model,
  ...
  fwrite(
    ...
    here(
      "CEoutput/ModelTwo",
      "Winter_MXL_ModelTwo_PrefSpace_Unconditionals.csv"
    )
  )
...
apollo_conditionals(Model,
  ...
  fwrite(
    ...
    here(
      "CEoutput/ModelTwo",
      "Winter_MXL_ModelTwo_PrefSpace_Conditionals.csv"
    )
  )

# **********************************************************************************
#### END OF SCRIPT ####
## Next step: 05_Winter_MXL_ModelThree_PrefSpace.R
# **********************************************************************************
