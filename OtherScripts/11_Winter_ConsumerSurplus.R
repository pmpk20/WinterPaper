...
## Imports WTP:
WTP <- here("CEoutput/ModelOne", "Winter_MXL_ModelOne_PrefSpace_ConWTP.csv") %>% fread() %>% data.frame()
...
CS_SQToMedium_Individual <- (-log(abs(-Model$estimate["asc_C"]+
                                        WTP$b_Smell.post.mean+
                                        WTP$b_Sound.post.mean+
                                        WTP$b_Colour.post.mean+
                                        WTP$b_Deadwood.post.mean))/MedianTax)


CS_SQToHigh_Individual <- (-log(abs(-Model$estimate["asc_C"]+
                                      WTP$b_Smell2.post.mean+
                                      WTP$b_Sound2.post.mean+
                                      WTP$b_Colour2.post.mean+
                                      WTP$b_Deadwood2.post.mean))/MedianTax)
