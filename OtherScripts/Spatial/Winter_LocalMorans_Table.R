#### RELATE Winter Paper ####
## Function: Local Moran Hotspot Statistics
...
RowLabels <- c("Tax",
               "Colour: Medium","Colour: High",
               "Smell: Medium","Smell: High",
               "Sound: Medium","Sound: High",
               "Deadwood: Medium","Deadwood: High")


# Rearrange a little
LocalMoransTable <- cbind("Variable"=rep(RowLabels,each=2),
      LocalMoransTable)


colnames(LocalMoransTable) <- c("Variable","Low:Low","Not Signif.","X5","High:High")


## Output to screen in a nice format for making tables in word
LocalMoransTable %>% write.csv(quote=F,row.names=F)

## Output to a discrete file if that's helpful
write.table(LocalMoransTable,
            here("OtherOutput/Spatial","LocalMorans.txt"),
            sep=",",quote=F)
